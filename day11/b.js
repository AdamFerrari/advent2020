...
class Board {
    ...
    findOcc(i, j, ydir, xdir) {
        i = i + ydir;
        j = j + xdir;
        while ( (i>=0) && (i < this.numRows()) && (j>=0) && (j < this.numCols()) ) {
            if(this.get(i,j)==board_full) return 1;
            if(this.get(i,j)==board_empty) return 0;
            i = i + ydir;
            j = j + xdir;
        }
        return 0;
    ...
    countOccDir(i, j) {
        let ret =  (
            this.findOcc(i,j,-1,-1) + this.findOcc(i, j, -1, 0) + this.findOcc(i, j, -1,1) +
            this.findOcc(i,j, 0, -1) +                            this.findOcc(i,j,0,1) +
            this.findOcc(i,j,1,-1) +  this.findOcc(i,j,1,0) +     this.findOcc(i,j,1,1)
        );
        //console.log("("+i+","+j+") = "+ret);
        return ret;
